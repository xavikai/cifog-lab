...
**Animation Lab** (`labs/animation/`) teaches the bouncing ball in three stages with a Blender-style workspace: a side camera view with Motion Path dots (one per frame) and optional ghosts, a Graph Editor with a channel list and a sidebar (Active Keyframe, bounce heights and frame counts), and a Timeline with playback. F-curves behave like Blender's: Constant, Linear and Bezier interpolation, and Free, Aligned, Vector, Automatic and Auto Clamped handles (automatic handles become Aligned when dragged). Keys and handles are selected with the left mouse button, box-selected by dragging, moved by dragging or with G (X/Y to constrain), and edited with I, X, T, V, A/Alt A, Home, arrows, Space and Ctrl Z, only while the mouse is over the workspace. **Timing**: from robotic Linear keys to eased tops, sharp Vector contacts, lower bounces and shorter bounces. **Squash & Stretch**: Z Scale keys at the contacts and around them, round at the top, and a Maintain Volume toggle (X Scale = 1/√Z Scale). **Weight**: turn a rubber ball into a bowling ball, give a beach ball hang time with long handles, and match a physically simulated bounce (score shown live). Each step is checked automatically, can show a solution and can be undone.
...
- `docs/animation-teaching-notes.md`, `docs/csharp-objects-teaching-notes.md`, `docs/skin-weights-teaching-notes.md`: teaching notes for the newer labs.
